...
from torchtune.models.qwen3._component_builders import lora_qwen3_self_attention
...
def dots1(
    ...
    """
    Build the decoder associated with the dots1 model. This includes:
    - Token embeddings
...
def dots1_mlp(dim: int, hidden_dim: int, quantize_base: bool = False) -> FeedForward:
    """
...
def dots1_moe_mlp(
    ...
    shared_expert = (
        dots1_mlp(dim=dim, hidden_dim=hidden_dim*num_shared_experts) if num_shared_experts else None
    )
    return MoE(
        experts=experts,
        router=router,
